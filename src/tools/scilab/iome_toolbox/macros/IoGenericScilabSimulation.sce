...
function [simid]=RequestSimulation(simfile, elist)
//function [status]=InitIOME(simname, configname, statename,port,server)
  nargin=length(elist);
  if nargin>0 then
    server=elist(1);
  ...
  //returns
  try
    scommand=sprintf("iogs requestsimulation %s %d %s", simfile, port,server);
    result=unix_g(scommand);
    simid=msscanf(result,'%d');
  catch
    disp('RequestSimulation Error!');
    simid=-1;
  end
endfunction

function [simid]=RunRequestedSimulation(elist)
//function [status]=InitIOME(simname, configname, statename,port,server)
  nargin=length(elist);
  if nargin>0 then
    server=elist(1);
  ...
  try
    ...
    disp('RunRequestedSimulation Error!');
    simid=-1;
  end
endfunction
...
//function [status]=InitIOME(simname, configname, statename,port,server)
  ...
  //returns
  try
    scommand=sprintf("iogs setsimulationstatus %d %d %d %s", newstatus, id, port,server);
    result=unix_g(scommand);
    status=msscanf(result,'%d');
  catch
    disp('SetSimulationStatus Error!');
    status=-1;
  end  
endfunction
