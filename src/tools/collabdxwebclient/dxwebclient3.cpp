...
Widget myWindow,myForm, theparent, thebar, theframe,updateserverbutton, updatelocalbutton, thefilebutton,theparamupdatebutton, theparamupdatemenu, thefilemenu, thehelpmenu,helpbutton, quitbutton, paramupdatecontrolbutton;
...
void uselocal_handler(DXLConnection *conn, const char *name, const char *value,    void *data)
{
  printf("uselocal handler\n"); 


			m_iuselocal=atoi(value);
   if(m_iuselocal==1)
   {
	...
	if(m_svoutputs   && (m_iupdateserver ==1))
	{
		  for(j=0; j<m_nvoutputs; j++)
		  {
		  	if(strcmp(m_svoutputs[j], name)==0)
		  	{
				...
				struct fdata sdata;
				sdata.__ptr=vval;
				sdata.__size=vecsize;
                m_ilenvvals[j]=vecsize;
				soap_call_ns__setparamvec(&m_soap, m_server, "", sname,sdata,vecsize, &status );
                free(vval);
...
XmStringFree (str5); 
    ...
    thehelpmenu = XmVaCreateSimplePulldownMenu (thebar, "help_menu", 2, help_menu_cb,
    ...
    /*get input list from server*/
		...
		if(m_iuselocal==0)
		{
			...
			for(i=0; i<m_nfinputs;i++)
			{
				...
				//printf("%d %s gs val svalue=%s\n",i,m_sinputs[i],soapresp);
		    	DXLSetValue(conn, m_sfinputs[i], m_sifvalues[i]);
		    	//DXLExecuteOnChange(conn);
		    	//SyncAfterExecute(conn);
			...
			for(i=0; i<m_niinputs;i++)
			{
		    	...
		    	DXLSetValue(conn, m_siinputs[i], m_siivalues[i]);
		    	//DXLExecuteOnChange(conn);
		    	//SyncAfterExecute(conn);
			...
			for(i=0; i<m_nsinputs;i++)
			{
		    	...
		    	DXLSetValue(conn, m_ssinputs[i], m_sisvalues[i]);
		    	//DXLExecuteOnChange(conn);
		    	//SyncAfterExecute(conn);
