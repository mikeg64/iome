...
if ~isa(dataIn,'double'); dataIn=double(dataIn); end

%% Define parameters
numScales                           = 55;       % maximum distance of analysis
stopCriterion                       = 0.03;     % stop criterion difference between steps
...
if levs>1
    ...
    if nargout==4
        ...
        for currLev=1:levs
            %[dataOut_S(:,:,currLev),errSurface_S(:,:,currLev)]=shadingCorrection(reduceu(dataIn(:,:,currLev),subSampl/2)); %#ok<AGROW>
            [dataOut_S(:,:,currLev),errSurface_S(:,:,currLev)]=shadingCorrection(dataIn(1:subSampl:end,1:subSampl:end,currLev)); %#ok<AGROW>
            %errSurface(:,:,currLev) = expandu(errSurface_S(:,:,currLev),subSampl/2);         %#ok<AGROW>
        end
        %dataOut = dataIn - errSurface(1:rows,1:cols,:);
        dataOut=dataOut_S;
    ...
    for cStep=1:2:numScales
        ...
        y_scaleMax(:,:,cStep)=y_LPF;
        y_scaleMin(:,:,cStep)=y_LPF;
        ...
        y_scaleMax(1+cStep:rows-cStep,1+cStep:cols-cStep,cStep)=max(y_scaleMax(1+cStep:rows-cStep,1+cStep:cols-cStep),tempAvMax);
        y_scaleMin(1+cStep:rows-cStep,1+cStep:cols-cStep,cStep)=min(y_scaleMin(1+cStep:rows-cStep,1+cStep:cols-cStep),tempAvMin);
        ...
        yMin=min(y_scaleMin(:,:,1:2:cStep),[],3);
        yMax=max(y_scaleMax(:,:,1:2:cStep),[],3);
        %

        sizeFR_S=cStep;


        sizeFC_S=sizeFR_S;  filtG_small=gaussF(sizeFR_S,sizeFC_S,1);

        %---- Low pass filter to adjust the effects of high/low points
        yMin0=padData(yMin,ceil(sizeFR_S/2));
        yMin1=conv2(yMin0,filtG_small);
        yMin2=yMin1(sizeFR_S+1:end-sizeFR_S,sizeFC_S+1:end-sizeFC_S);
        yMax0=padData(yMax,ceil(sizeFR_S/2));
    ...
    %%
    %compare the gradients to decide which surface to keep (smallest)
