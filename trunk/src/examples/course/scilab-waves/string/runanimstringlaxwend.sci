...
uh = zeros(max(size(x)),max(size(y)));
vh = zeros(max(size(x)),max(size(y)));

u((ni+1)/2,(nj+1)/2)=b;
  for i = 2:ni-1
    for j = 2:nj-1
    
       uh(i,j)=(u(i+1,j+1)+u(i+1,j)+u(i,j+1)+u(i,j))/4;
       vh(i,j)=(v(i+1,j+1)+v(i+1,j)+v(i,j+1)+v(i,j))/4;
...
for n = 1:nt
  ...
  for i = 2:ni-1
    for j = 2:nj-1  
      vh(i,j) = ((v(i+1,j+1)+v(i+1,j)+v(i,j+1)+v(i,j))/4)-damp*v(i,j)-(dt/2)*dx*k*(4*u(i,j)-u(i+1,j)-u(i-1,j)-u(i,j+1)-u(i,j-1));//+force*dt*sin(forcefreq*t*%pi);
            uh(i,j)=uh(i,j)+(dt/2)*(v(i+1,j+1)+v(i+1,j)+v(i,j+1)+v(i,j))/4;
    end;
  end;
  ...
  //end;
   ...
   for i = 2:ni-1
    for j = 2:nj-1  
      v(i,j) = v(i,j)-(dt)*dx*k*(4*uh(i,j)-uh(i+1,j)-uh(i-1,j)-uh(i,j+1)-uh(i,j-1));//+force*dt*sin(forcefreq*t*%pi);
      u(i,j)=u(i,j)+(dt/2)*(v(i+1,j+1)+v(i+1,j)+v(i,j+1)+v(i,j))/4;
    end;
  end;
...
//  s.data.y = u(:,25);
  ...
  clf;
  xset('pixmap',1);
plot2d(x,u(:,25),rect=[0 -4 1.0 4.0]);
